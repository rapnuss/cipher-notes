...
const buildWidget = (lines: string, sep: string, pos: number) => {
  let decoItem,
    result = [],
    item = lines.split(sep)
  for (let j = 0; j < item.length; j++) {
    pos += item[j]!.length
    if (linkSchema.safeParse(item[j]!).success) {
...
export const TextToLink = ViewPlugin.fromClass(
  class {
